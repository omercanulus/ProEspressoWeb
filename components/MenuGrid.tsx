...
const MenuGrid: React.FC = () => {
  ...
  const menuData = [
    {
      title: t('menu.espressoBar.title'),
      description: t('menu.espressoBar.description'),
      icon: "fa-bolt",
      items: ["Signature Espresso", "Ristretto", "Americano", "Cortado", "Macchiato"]
    },
    {
      title: t('menu.milkCoffees.title'),
      description: t('menu.milkCoffees.description'),
      icon: "fa-droplet",
      items: ["Flat White", "Caffe Latte", "Cappuccino", "Mocha", "Spanish Latte"]
    },
    {
      title: t('menu.brewBar.title'),
      description: t('menu.brewBar.description'),
      icon: "fa-flask-vial",
    ...
    <section id="menu" className="py-32 bg-cream-25 text-coffee-900">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24">
          <h2 className="text-coffee-500 text-[10px] font-bold tracking-[0.5em] uppercase mb-4">{t('menu.label')}</h2>
          <h3 className="text-5xl md:text-6xl font-black tracking-tighter text-coffee-900 uppercase">{t('menu.title1')} <span className="text-coffee-500">{t('menu.title2')}</span></h3>
          <div className="w-16 h-[2px] bg-coffee-500 mx-auto mt-8 opacity-30"></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {menuData.map((section, idx) => (
            <div key={idx} className="group bg-white p-12 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-cream-100 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] transition-all duration-700 relative overflow-hidden rounded-sm">
              ...
              <p className="text-coffee-800/60 text-[13px] leading-relaxed mb-10 font-serif italic">{section.description}</p>
